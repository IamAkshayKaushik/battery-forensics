 ...
 * Privileged dumpsys evidence (when present) is passed into [RuleContext] for Measured/Derived rules.
 */
class DiagnosticsEngine(
    ...
    fun investigate(
        ...
        privileged: PrivilegedEvidence? = null,
        baselineSamples: List<MonitoringSample> = emptyList(),
    ): InvestigationResult {
        val diagnoses = ruleEngine.evaluate(
            RuleContext(
                samples = samples,
        ...
        return InvestigationResult(
            ...
            privilegedUsed = privileged?.hasAnyData == true,
        )
    }
}
...
data class InvestigationResult(
    ...
    val privilegedUsed: Boolean = false,
)
